...
export async function GET(request: Request) {
  ...
  if (!contentTypeId) {
    return NextResponse.json({ error: 'Content type ID is required' }, { status: 400 });
  }
  ...
  try {
    const data = await cms.getLatestEntries(
      ...
      Number(limit),
      order as (OrderFilterPaths<EntrySys, "sys"> | "sys.contentType.sys.id" | "-sys.contentType.sys.id" | `fields.${string}` | `-fields.${string}` | `fields.${string}.sys.id` | `-fields.${string}.sys.id`)[]
      );
